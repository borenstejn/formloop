#!/usr/bin/env bash
# formloop installer (macOS / Linux)
#
# Most users want `npx formloop` — that's the one-liner.
# This script is for agent platforms that need the skill files symlinked into
# specific dirs (Codex, OpenCode, Gemini CLI, Cursor, Copilot, etc.).
#
# Usage:
...
#
# Curl-pipe:
#   curl -fsSL https://raw.githubusercontent.com/borenstejn/formloop/main/install.sh | bash
#   curl -fsSL https://raw.githubusercontent.com/borenstejn/formloop/main/install.sh | bash -s codex
#
# Environment:
#   FORMLOOP_REPO_URL  Override clone URL (default: official GitHub repo)
#   FORMLOOP_DIR       Override clone destination (default: $HOME/.formloop/repo)

set -euo pipefail

REPO_URL="${FORMLOOP_REPO_URL:-https://github.com/borenstejn/formloop.git}"
REPO_DIR="${FORMLOOP_DIR:-$HOME/.formloop/repo}"
PLUGIN_LINK="$HOME/.formloop-plugin"

# Platform table — id|skills-target-dir|style
#   "per-skill": one symlink per skill into the target dir
#   "folder":    one symlink for the whole skills/ dir, named "formloop"
platforms_table() {
  cat <<EOF
...
skills_root() { printf '%s\n' "$REPO_DIR/formloop-plugin/skills"; }
...
link_skills() {
  ...
  case "$style" in
    per-skill)
      ...
      ;;
    folder)
      ln -sfn "$root" "$target/formloop"
      printf '  ✓ %s → %s\n' "$target/formloop" "$root"
      ;;
    *)
    ...
    per-skill)
      if [[ -d "$(skills_root)" ]]; then
        ...
        for link in "$target"/*; do
          ...
          [[ "$resolved" == *"/formloop-plugin/skills/"* ]] || continue
          rm -f "$link"
        done
      fi
      ;;
    folder)
      [[ -L "$target/formloop" ]] && rm -f "$target/formloop"
      ;;
  esac
}
...
link_plugin_root() {
  if [[ -L "$PLUGIN_LINK" || -e "$PLUGIN_LINK" ]]; then
    ...
    ln -s "$REPO_DIR/formloop-plugin" "$PLUGIN_LINK"
    printf '  ✓ %s → %s\n' "$PLUGIN_LINK" "$REPO_DIR/formloop-plugin"
  fi
}

print_server_hint() {
  printf '\nformloop needs a local HTTP server to render forms. Two options:\n'
  printf '\n  Option A — npx (simplest):\n'
  printf '    npx formloop                          # boots server on 127.0.0.1:3847\n'
  printf '\n  Option B — from this checkout:\n'
  printf '    cd %s/apps/server\n' "$REPO_DIR"
  printf '    npm install && npm run build && PORT=3847 npm start\n'
  printf '\nThe agent skill assumes the server is reachable at http://127.0.0.1:3847.\n'
}
...
cmd_install() {
  ...
  printf '\n✓ Installed formloop for %s\n' "$id"
  printf '  Restart your CLI or IDE to pick up the skill.\n'
  print_server_hint
  if [[ "$id" == "vscode" ]]; then
  ...
  if [[ "$id" == "claude" ]]; then
    printf '\n  Tip: in Claude Code you can also use the plugin marketplace:\n'
    printf '       /plugin marketplace add borenstejn/formloop\n'
    printf '       /plugin install formloop\n'
  fi
}
...
Most users only need:
  npx formloop                       Boots the server on 127.0.0.1:3847.
  /plugin marketplace add borenstejn/formloop   In Claude Code, native plugin install.

Environment:
  FORMLOOP_REPO_URL  Override clone URL (default: official repo)
  FORMLOOP_DIR       Override clone destination (default: \$HOME/.formloop/repo)
USAGE
}
...
main() {
  case "${1:-}" in
    ...
    --uninstall)
      ...
      local id="${1:-}"
      if [[ -z "$id" ]]; then
        printf '%s requires a platform argument.\n' "--uninstall" >&2
        exit 1
      fi
      cmd_uninstall "$id"
      exit 0
